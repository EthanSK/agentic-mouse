...
## 2026-09-03 — Add the retained VS Code selection to Codex through one exact bridge route

**Trigger:** Ethan asked for an `Add to chat` Codex-mode card in the broken Edit Queued Message slot, physical cell 8 on both mice, while keeping the separate Chats Selection wheel card unchanged.

**Finding:** The installed OpenAI VS Code extension exposes `chatgpt.addToThread`, but Agentic Mouse cannot invoke an arbitrary VS Code command directly and Codex is normally frontmost while this app mode is open. VS Code still retains its active editor selection in the background, so this one action needs a narrow exception to the bridge's usual frontmost-only rule. Live installation also exposed a separate packaging trap: the first `/Applications` bundle reported the new v1.0.164 metadata and passed signature verification while its executable hash still differed from the signed candidate and its HUD still rendered the old Edit action. Version and signature alone did not prove the candidate executable was actually installed.

**Fix:** Replace only Codex action cell 8 with `Add to chat` and open only `vscode://ethansk.agentic-mouse-vscode-bridge/codex/add-to-chat`; bridge extension v0.1.2 invokes the exact `chatgpt.addToThread` command. Permit background VS Code only for this action so it can read the retained editor selection. Install by moving the old app bundle completely away before copying the signed candidate into the now-empty `/Applications/AgenticMouse.app` path. (Codex task: 01a068dc-c698-7312-bc0b-6221c39286e4)

**Guard:** Keep physical cell 11 as `Chats Selection + Wheel`, keep every other VS Code bridge action frontmost-only, and reject arbitrary command identifiers. Pin the exact canonical cell, both printed mouse labels, exact URI, background-selection exception, and no keyboard-transport fallback in tests. After every install, compare the installed main-executable hash with the candidate before trusting the version, signature, or HUD footer, then require full-display HUD pixels and literal physical acceptance separately.

**Verification:** The complete clean gate passed 700 Swift tests, seven VS Code bridge tests, six Musixmatch tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, packaging/version contracts, and shell syntax. The standalone bridge package v0.1.2 is installed and its installed `bridge.js` SHA-256 matches the repository and packaged VSIX at `587c9e1dbc4366b2bdf089a66720afea5055a3807333f6a59f4800c111c06281`. VS Code's old extension host was restarted without closing its editor window, unsaved files, or terminals; the new host activated `ethansk.agentic-mouse-vscode-bridge` from the installed v0.1.2 registration. Developer-ID-signed Agentic Mouse v1.0.164 (build 170) is installed as main PID 81788 with supervisor PID 81795; its installed and candidate executable SHA-256 values match at `87157605266ebd18c1c8347b4892163adde38e0e6e97b09e0dfef7e68ca56723`. Full-display screenshots on the AVT GC553G2, Built-in Retina Display, and LS27A800U current Spaces showed the complete v1.0.164 Razer and Corsair Codex legends with `Add to chat · Razer 8` / `Add to chat · Corsair 8` and the unchanged `Chats Selection + Wheel · Razer 11` / `Chats Selection + Wheel · Corsair 11`; a final three-display capture proved the prior hidden-HUD state was restored. OBS++ main PID 98893 and active recording mux PID 6842 remained unchanged. Literal physical acceptance remains Ethan-owned: select text in VS Code, open Codex mode, press printed Razer 8 or Corsair 8, and confirm the selection enters the current Codex chat.
