...
**Finding:** A rapid-double classifier must defer the single action until its decision window expires, so it cannot make ordinary navigation feel immediate. Karabiner's release lifecycle can keep cell 8 armed for a chord without that timer, but `to_if_alone` is the wrong primitive because Karabiner cancels it after a long hold or another input. A source-isolated held-state variable plus `to_after_key_up` preserves an unlimited physical hold and lets the same mouse's cell 7 consume the pending Next action.

**Fix:** Emit top-level VS Code F17 Previous Change and F13 Next Change only on physical release. While the same mouse's cell 8 state is armed, consume one cell-7 press as non-repeating F18 Stage + Next, mark that hold consumed, and suppress both ordinary Enter and the pending F13. Keep separate Corsair and Razer state variables so the two mice cannot form a chord. Update the persistent Default HUD from the live frontmost VS Code context, including Razer's mirrored printed Enter button 9. Keep the explicitly entered VS Code child page's existing double gestures unchanged. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Never restore a top-level double-click decision window or use `to_if_alone` for this held chord. Keep the VS Code chord rule before the base Enter rule, preserve ordinary Forward/Back/Enter outside the exact same-source hold, clear the held state on cell-8 release, and pin both exact-device variable names, release-only F17/F13 outputs, F18 consumption, no F19 top-level output, and source-specific HUD copy in tests.

**Verification:** The focused exact-device generator and Default HUD context tests passed, followed by the complete clean gate: 701 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both generated Karabiner lints, packaging/version contracts, and shell syntax. Developer-ID-signed Agentic Mouse v1.0.161 (build 167) is installed as main PID 14388 with supervisor PID 14404, executable SHA-256 `36ab1c59166e2641fa497c8ae03b532ff9695bd52edc2c27272de2d13fa22f9d`, supervisor SHA-256 `688e8a581b87ba1ca9b56f1038122a54814c613c1cf8c8ce3145922ab5f0d691`, main CDHash `3d1b2eaf33f46b736f32ff79355a0b7c0b2e882a`, supervisor CDHash `1c0fe6686940c23f5959b3532350ba0190ae9680`, Team ID `T34G959ZG8`, Accessibility granted, iCUE connected, self-recovery enabled, and exclusive ownership of the mode-0600 command socket. The installed iCUE SDK and live app configuration remain byte-identical at SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe` and `aa1499d88bd34875dc11f9a2873165d09cd2323c590126f425da4fd72e3189be`. The live Karabiner profile is SHA-256 `08cc0c02215478e6dec69f935d7e1ef573ec980b0e25824288a883efd79abaf6`: its generated six-rule Agentic Mouse block matches the source exactly, both VS Code exact-device layers contain only Previous, Next, and the source-isolated Stage + Next chord, and all three non-Agentic rules match the pre-install profile exactly. Full-display pixel captures on the AVT GC553G2, Built-in Retina Display, and LS27A800U current Spaces proved one complete v1.0.161 Corsair Default legend per display with `Previous Change`, `Next Change · Hold + 7 to Stage`, and `Enter · 8 held: Stage + Next`; the pre-install zero-HUD state was then restored and visually verified on all three displays. OBS++ main PID 23157 and active recording mux PID 99307 remained unchanged. The exact v1.0.160 bundle and prior Karabiner profile are preserved in `Rollbacks/`. Literal physical acceptance remains one Previous release, one Next release, and one 8-held + 7 press from each mouse, with the Razer chord physically printed 8 + 9.
...
**Finding:** The accepted HUD source already centralizes typography, card geometry, spacing, borders, material, and dynamic status content. Rebuilding a separate compact layout would create a second geometry source and let Default, runtime-mode, and Keypad HUDs drift. Making the reference panel itself mouse-interactive would also break its established click-through contract and could take input from the application Ethan is using. The first installed implementation used a `CALayer` affine transform after shrinking the window, so SwiftUI still clipped drawing to the smaller host bounds and Ethan saw only part of the HUD; fitting-size assertions alone did not exercise rendering geometry and failed to catch it.

**Fix:** Keep the accepted 705-point layout as the 100% source, map its full unscaled AppKit bounds into the smaller window frame, and default the shared scale to 50% in each axis so the visible panel occupies one quarter of the former area. Persist one clamped 35–100% scale in `UserDefaults` and observe it from both HUD presenter families. Keep every reference panel click-through and non-activating; attach one separate non-activating vertical control panel to each visible HUD's handed bottom corner, reveal only the control belonging to the hovered HUD, and interpret upward drag as larger and downward drag as smaller. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Never maintain parallel compact card metrics, scale only the outer frame while leaving content unscaled, apply a content-layer transform inside an already-shrunk host, make the reference panel accept clicks, or store separate sizes per mode, mouse, or display. Pin the live frame-to-full-bounds mapping in a geometry test, reconcile every visible panel from the shared store, keep the scale control hover-only, and prove persistence from a fresh store instance rather than relying on an in-memory resize.

**Verification:** Five focused scale tests prove exact half-width and half-height defaults, full source bounds mapped into the smaller frame, persistence across store instances, bounded invalid input, and that only the non-activating scale control accepts pointer events. Six focused Space lifecycle tests pass. One timing-sensitive Space reattachment assertion failed once during the first full run, then passed in isolation and the immediate complete clean rerun passed with 698 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, packaging/version contracts, and shell syntax. The real click-through presenter was then held open and full-display pixels were inspected on all three current displays: each showed the complete 4×3 grid at 353×188/189 points with no clipping, and hovering the Built-in-display copy made only its 30×78 scale control visible. Developer-ID-signed Agentic Mouse v1.0.157 (build 163) is installed with the same embedded iCUE SDK, live application configuration, live Karabiner configuration, command-socket mode, and zero-HUD open state as the preceding build. The clipped v1.0.155 bundle is preserved at `Rollbacks/AgenticMouse-v1.0.155-build161-clipped-before-v1.0.157.app`. A literal installed-runtime drag and relaunch-persistence acceptance still require the app's first real physical input after relaunch; synthetic mouse movement and socket commands correctly cannot satisfy that security boundary.
...
**Finding:** The path-only implementation waited until the rapid double press to reopen the saved image, decode it, generate TIFF, write the file URL and image representations to the pasteboard, and then send Command-V. The 250 ms lease delay happened after Command-V and protected restoration; it was not the cause of the initial paste latency. The expensive image read and conversion on each paste was avoidable while the screenshot still owned the normal clipboard.

**Fix:** As soon as the bounded saved-file resolver identifies one readable screenshot, copy its exact file URL, original image bytes, TIFF, and private ownership marker to the general pasteboard and retain the exact saved path. While that marker and change count still own the pasteboard, a rapid same-source double press sends Command-V directly without rereading the image or rewriting the clipboard. If VoiceInk, the user, or another app replaced the clipboard, temporarily install the retained screenshot, paste it, and restore the newer clipboard only while the temporary lease still owns it. Keep polling when the file exists but is not yet readable instead of reporting a false failure while macOS is still writing it. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** A fresh screenshot is intentionally the newest normal clipboard item and stays available for manual Command-V. Never restore clipboard content from before the capture. Recovery paste must still preserve content written after the screenshot, and every clipboard write newer than the temporary recovery lease must win. Do not remove the retained path, take a second screenshot, shorten the 60-second slow-destination allowance, or make an unreadable partial file final.

**Verification:** The focused Screenshot suite passed 25 tests, including one clipboard write at saved-file resolution, zero rewrites or restore timers for repeated fast-path pastes, retry after a temporarily unreadable file, and VoiceInk replacement surviving the path-based recovery paste. The complete clean gate passed 691 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, packaging/version contracts, and shell syntax. Developer-ID-signed Agentic Mouse v1.0.152 (build 158) is installed as main PID 12467 with supervisor PID 12489, executable SHA-256 `1697dd113ecc0f523cfd299217087695898428e2787c3725a91eca69efb9deb8`, CDHash `e0cd2f37dad0d725b5f1c0eac414ebeeeefb87ea`, Team ID `T34G959ZG8`, Accessibility granted, iCUE connected, self-recovery enabled, and exclusive mode-0600 command-socket ownership. The embedded iCUE SDK remains byte-identical at SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe`. The exact prior app is preserved at `Rollbacks/AgenticMouse-v1.0.151-build157-before-v1.0.152.app`, and its replaced installed copy is recoverable from Trash. OBS++ recording mux PID 26937 and Rekordbox remained on their original work throughout. Literal physical capture, manual paste, and perceived latency remain Ethan-owned acceptance because an agent-driven native selection would take over his live pointer.
...
**Finding:** Physical cell 5 previously emitted ordinary Forward immediately from Karabiner, so Agentic Mouse could not tell whether a later same-source cell-6 wheel ratchet should suppress that navigation. Cell 6 already carried a source-aware press/release lifetime and the established bridge already selected YouTube targets without focusing Chrome.

**Fix:** Route base-layer physical cell 5 press/release into Agentic Mouse and delay its ordinary auxiliary-button Forward click until an unused release. When the same source also holds cell 6, reinterpret accepted scrub ratchets as fixed YouTube volume steps: physical wheel up is +5 percentage points and physical wheel down is −5. A used volume ratchet suppresses both delayed Forward and cell 6's click-to-rewind; an unused button retains its old release action in either release order. Keep the VS Code cell-5 Better Git override unchanged. While cell 5 is held, update only an already-visible source Default legend with `YouTube Volume held` and `YouTube Volume + Wheel`, using the real printed scrub number for each mouse. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Keep the chord same-source and top-level only. Cell 5 alone must not consume ordinary wheel input; opposite mice must not combine; lifecycle teardown must clear both holds without synthesizing either release action; volume dispatch must remain fixed at ±5%, target only a genuinely playing YouTube video, preserve play/pause and focus, and drop rather than replay a ratchet when the bridge is unavailable. Tests pin both press orders, both release orders, old Forward and rewind behavior, source isolation, direction, exact Karabiner commands, bridge notifications, and dynamic HUD copy.

**Verification:** Focused Swift coverage passed 78 tests and the exact-device Karabiner generator suite passed all 17 tests. The complete clean gate passed 690 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, packaging/version contracts, shell syntax, and diff hygiene. The bridge's focused volume, scrub, speed-hold, VoiceInk finish, and watchdog suites passed; both extension scripts passed syntax checks; and its signed helper/native build completed. Developer-ID-signed Agentic Mouse v1.0.150 (build 156) is installed as main PID 15454 with executable SHA-256 `ae94a0dafff6c68da99b30269463191d8474831a0dc3e9948a1ffad701c6c87a`, CDHash `692079399dda76298bc807ae21e0148f4618a2d3`, Team ID `T34G959ZG8`, Accessibility granted, the audited iCUE SDK loaded, and exclusive mode-0600 command-socket ownership. Its matching signed runtime supervisor is enabled as PID 15459 with executable SHA-256 `298d8c8d03432979c6daaa3a5903f4b90d889dcf40c59eb21ebb3f50d0efb18d`, CDHash `e87222bc3f5d57ba169cb5c5e72831e2a99b6024`, and the same Team ID. The embedded iCUE SDK and live config remain byte-identical at SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe` and `aa1499d88bd34875dc11f9a2873165d09cd2323c590126f425da4fd72e3189be`; the live Karabiner hash changed deliberately from `9b2dfb6498d9e0d653200d7ccaa22fc03aaab6f99506a25e7ff4ec34759ead68` to `b2a3eea496fdbe7620ad5e975f1434be21308765d2fd8fd846395453c90e7fed` while preserving all three non-Agentic rules. Direct HUD captures and full-display pixel screenshots across every connected display's current Space proved one v1.0.150 Corsair legend per display with `YouTube Volume held` on cell 5 and `YouTube Volume + Wheel` on cell 6; the pre-install hidden-HUD state was restored without emitting Forward or changing YouTube volume. The matching signed YouTube bridge helper is installed as PID 27871 with executable SHA-256 `6d23acef78228cb6a3ad7852e9ac96c02567e640f6d24c53bfe4e85bc34ec6f2` and CDHash `18b3002d3190c5e42eba8bfd64732bc32a5fc132`. Chrome's agent security policy blocked opening `chrome://extensions`, so Ethan must manually reload the existing unpacked extension before the new background/content-script volume route is live. Literal physical volume direction, five-point cadence, and release suppression remain Ethan-owned acceptance after that reload. OBS++ recording mux PID 26937 and the Rekordbox sync watcher remained on their original work throughout installation and visual proof.

## 2026-09-01 — Observe Screenshot selections before the native overlay consumes them

**Trigger:** Ethan reported that installed build 154 still saved a selected-area screenshot but left the Default HUD on `Screenshot`, so the next rapid double-press started another capture instead of showing and performing `Screenshot · 2× Paste`.

**Finding:** The path-owned paste state was correct after saved-file resolution, but build 154 still depended on `NSEvent.addGlobalMonitorForEvents` to decide whether the native selection completed. The Screenshot overlay can consume the successful selection's AppKit global mouse-up as well as its drag events. Public `CGWindowList` polling then observed the overlay disappear with no recorded mouse-up, classified the real capture as cancellation, and never entered saved-file resolution even though macOS wrote the PNG.

**Fix:** Observe left mouse-down, drag, mouse-up, and Escape with a short-lived listen-only Core Graphics session event tap at the head of the event stream. Keep the existing public `screencapture` window poll for external cancellation and retain the exact 60-second saved-file resolver, path-owned HUD state, same-source double-press classifier, short restoring pasteboard lease, and current-Space HUD reattachment. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Never restore AppKit's global event monitor for this lifecycle or infer successful capture solely from the overlay disappearing. The event tap must remain listen-only, bounded to the active interaction, and cover every selection boundary without swallowing input.

**Verification:** The focused Screenshot suite passed 24 tests without compiler warnings, including the listen-only tap's exact event mask and the existing moved mouse-up, cancellation, path retention, double-press paste, clipboard restoration, and VoiceInk-wins cases. The complete clean gate passed 682 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, packaging/version contracts, and shell syntax; the updated `configure-mice` skill passed validation. Developer-ID-signed Agentic Mouse v1.0.149 (build 155) is installed as main PID 75635 with supervisor PID 75675, executable SHA-256 `d74cc2a10a383480c80730fea768dc01480ebeaccbd1ec4685d19d4e91699b6b`, CDHash `717f773edcddb3891b9096bd5dc10dd6aef42003`, Team ID `T34G959ZG8`, Accessibility granted, the audited iCUE SDK loaded, and exclusive mode-0600 command-socket ownership. The embedded iCUE SDK, live config, and live Karabiner configuration remain byte-identical at SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe`, `aa1499d88bd34875dc11f9a2873165d09cd2323c590126f425da4fd72e3189be`, and `9b2dfb6498d9e0d653200d7ccaa22fc03aaab6f99506a25e7ff4ec34759ead68`. A live native drag created exactly one new path-owned PNG; full-display pixel captures then showed `Screenshot · 2× Paste` on one current-Space Corsair HUD per connected display. Ethan physically rapid-double-pressed and pasted the retained screenshot three times, confirming the real mouse path no longer starts another capture. The pre-install zero-HUD state was restored, the blank TextEdit test document was closed, and OBS++ recording mux PID 26937 remained on the same file throughout. The exact prior build is preserved at `Rollbacks/AgenticMouse-v1.0.148-build154-before-v1.0.149.app`, with the replaced application also recoverable from Trash.

## 2026-09-01 — Make Keys media navigation one action per wheel ratchet

**Trigger:** Ethan asked to turn Keys mode's direct Next Track button into a two-way held-wheel control: physical wheel up for Next Track and physical wheel down for Previous Track, using the existing ratchet debounce.

**Finding:** Keys cell 9 previously emitted `scan_next_track` directly from Karabiner on button press. Adding a wheel route only inside Agentic Mouse would therefore skip once on press before the user moved the wheel. Both exact-device mode transports already preserve the cell's press/release lifetime, so the direct consumer-key output can be removed and the same shared `WheelChordStateMachine` can own media dispatch.

**Fix:** Keys physical cell 9 now arms `Tracks + Wheel` until release. The fixed 80 ms leading-edge filter collapses duplicate raw events from one detent without extending the window, allowing later ratchets during the same hold and immediate direction reversal. The normalized Quartz direction is deliberately inverted only at the media resolver because both accepted mouse routes deliver a physical upward ratchet as `.down`: physical up sends the native Next Track consumer key and physical down sends Previous Track. A wheel-free click sends one Next Track on the real physical release; any observed wheel input suppresses that click action, and teardown clears the chord without impersonating a release. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Keep this inside Keys mode on canonical cell 9 for both exact mice. Do not restore a press-time Next Track output, change another wheel family's polarity or cadence, alter the physical-cell crosswalk, or add an iCUE transport. Generated Karabiner must send only the existing source-specific press/release command for this cell while Keys is active.

**Verification:** The focused suites passed 15 Swift tests, including both native consumer-key cycles, exact Keys press/release arming, no direct Keys action, fixed-window duplicate filtering, a later same-direction ratchet, and immediate reversal. The complete clean gate passed 681 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, packaging/version contracts, and shell syntax; the updated `configure-mice` skill passed validation. Developer-ID-signed Agentic Mouse v1.0.148 (build 154) is installed as main PID 428 with supervisor PID 448, executable SHA-256 `f697b4c0af5ea84e51f646018f0ef5a3d3eebd8823cdc681528b2964f947a347`, CDHash `48d2a5a39ee029fd7f9e3c92f190d1696e433813`, Team ID `T34G959ZG8`, Accessibility granted, iCUE connected, and exclusive command-socket ownership. The embedded iCUE SDK and live config remain byte-identical at SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe` and `aa1499d88bd34875dc11f9a2873165d09cd2323c590126f425da4fd72e3189be`. The live Karabiner hash changed deliberately from `987b071fa392851b0cf7ed0efd6675d09aed50b1456db0e745291e4899852920` to `9b2dfb6498d9e0d653200d7ccaa22fc03aaab6f99506a25e7ff4ec34759ead68`; its six Agentic Mouse rules match the generated runtime and its three non-Agentic rules remain byte-equivalent. The exact prior app and Karabiner file are preserved under `Rollbacks/`, and the replaced app is recoverable from Trash. OBS++ recording mux PID 56552 remained active on the same file throughout. That historical acceptance covered only the wheel directions and cadence; the later wheel-free Next Track click uses the same native consumer-key executor and requires its own installed physical acceptance.

**2026-09-02 click follow-up verification:** Focused release, teardown, and media-state tests passed, then the complete clean gate passed 700 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, packaging/version contracts, and shell syntax. The updated `configure-mice` skill passed validation. Developer-ID-signed Agentic Mouse v1.0.158 (build 164) is installed as main PID 72080 with supervisor PID 72096, executable SHA-256 `5beaefd2b176f735e0dd4561d21f3ed2858b254ac190af30c5dccb7b8a3d30a0`, CDHash `648c7ef09b29f08d8dc0b1eee2bcbac8afdac299`, Team ID `T34G959ZG8`, Accessibility granted, iCUE connected, and exclusive mode-0600 command-socket ownership. The embedded iCUE SDK and live Karabiner configuration remain byte-identical at SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe` and `b2a3eea496fdbe7620ad5e975f1434be21308765d2fd8fd846395453c90e7fed`. The prior signed app is preserved at `Rollbacks/AgenticMouse-v1.0.157-build163-before-v1.0.158.app` and the replaced bundle remains recoverable from Trash. Literal click acceptance remains Ethan-owned: enter Keys and click physical cell 9 once for exactly one Next Track, then use the wheel and confirm release adds no second skip.

## 2026-08-31 — Keep selected-area Screenshot path-owned until explicit paste

**Trigger:** Ethan reported that VoiceInk could replace the pasteboard between a selected-area capture and the Screenshot button's rapid double-press paste. He also reported that externally dismissing the native crosshair could leave the HUD stuck in its capture state. Physical use of v1.0.146 then proved that dragging a selection did not arm paste at all: rapid double-press started another capture.

**Finding:** The native Shift-Command-4 interaction saves the authoritative image to the configured Screenshot directory, so keeping a screenshot resident on the general pasteboard after capture unnecessarily competes with VoiceInk. Arbitrary macOS apps still accept an image at the current cursor through Command-V rather than through a universal direct file-insertion API, so the correct boundary is a short pasteboard lease at the moment of paste, not no pasteboard use at all. A plain click without pointer movement dismisses the native crosshair without writing a file. The macOS overlay can consume global selection events, so requiring an observed AppKit drag event misclassified real captures as cancellation; the later build-154 learning above supersedes this section's earlier assumption that AppKit would always deliver mouse-down and mouse-up. The public `CGWindowList` owner `screencapture` exposes the bounded interaction while it is active. Live v1.0.145 verification also proved that closing the native overlay can strand fully rendered HUD panels on stale display Spaces even when their model remains active.

**Fix:** Snapshot the configured Screenshot directory before capture, resolve and retain the exact new saved image path without changing the pasteboard, and on rapid double-press snapshot every current pasteboard item before installing the exact file URL, original image representation, TIFF, and a private ownership marker. Send one process-targeted hardware-shaped Command-V to the then-frontmost app, then restore the old pasteboard after 250 ms only while the marker and change count still own it; a newer VoiceInk, user, or application write wins. Repeated screenshot pastes share the pre-first-paste snapshot. Classify an unmoved mouse-down/up as cancellation, any moved mouse-down/up or delivered drag event as completion, and Escape as cancellation while retaining the original 60-second saved-file resolver for a real selection. When the native overlay ends, recreate only explicitly active HUD presenters immediately and once after 500 ms so their panels reattach to every current display Space. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Do not take a second screenshot, hold the screenshot on the general pasteboard between capture and paste, restore over a newer clipboard write, require the native overlay to expose intermediate global drag events, infer completion from mouse-up without comparing its position with mouse-down, shorten the real capture's slow-destination allowance, or treat AX window counts/direct captures as current-Space visibility proof. Hidden legends must remain hidden when the screenshot overlay closes.

**Verification:** The corrected complete clean gate passed 677 Swift tests, including 23 Screenshot controller tests with the exact mouse-down/moved-mouse-up/no-drag-event regression, plus six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, both Karabiner lints, packaging/version contracts, and shell syntax. The updated `configure-mice` skill passed validation. Developer-ID-signed Agentic Mouse v1.0.147 (build 153) is installed as main PID 59772 with supervisor PID 59782, executable SHA-256 `50743507569075d7411861373c821216f31b2609c6220e5a64fb9c94e1beb52a`, CDHash `6c4f2650340092f4c05d1b5e8143bc7e5c4105f7`, Accessibility granted, iCUE connected, self-recovery enabled, and exclusive mode-0600 ownership of the command socket. Its embedded iCUE SDK, live config, and live Karabiner configuration remain byte-identical at SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe`, `aa1499d88bd34875dc11f9a2873165d09cd2323c590126f425da4fd72e3189be`, and `987b071fa392851b0cf7ed0efd6675d09aed50b1456db0e745291e4899852920`. A live native drag produced one new path-owned PNG, proving the installed lifecycle reached saved-file resolution instead of treating the selection as cancellation. The first synthetic selection was clipped to 4×2 pixels on the negative-coordinate Built-in display, so its TextEdit paste was not visually distinguishable; a second attempt was invalidated and stopped when Ethan's live focus moved to Chrome. Literal physical double-press paste remains Ethan-owned acceptance rather than a claimed visual success. Both task-created screenshots were moved recoverably to Trash, all Screenshot UI was dismissed, the disposable TextEdit document was closed without saving, the initial zero-HUD state was restored, and OBS++'s active mux remained PID 56552 throughout. The exact prior build is preserved at `Rollbacks/AgenticMouse-v1.0.146-build152-before-v1.0.147.app`, with the replaced application also recoverable from Trash.

## 2026-09-01 — Swap Utility Zoom and manual app selection

**Trigger:** Ethan asked to exchange the Utility `Zoom + Wheel` and `Choose App Specific` controls.

**Finding:** Both exact mice derive Utility actions and legends from one canonical `PhysicalCell` map. Zoom occupied canonical cell 2 while the manual app selector occupied canonical cell 11; changing the canonical aliases moves behavior, HUD labels, and both device-number translations together without changing either mouse transport.

**Fix:** Put `Choose App Specific` on Utility cell 2 and `Zoom + Wheel` on Utility cell 11. Preserve Zoom's existing wheel polarity and keep top-level cell 2's separate current-frontmost-app behavior unchanged. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Keep this as a Utility-only position swap. Do not change the physical-cell crosswalk, top-level current-app entry, app-child exit cells, Zoom direction, exact-device transports, or Karabiner configuration.

**Verification:** The focused Mode Picker suite and the complete 677-test Swift gate passed. Installed v1.0.147 contains the shared canonical swap; the live config and Karabiner file remained byte-identical because Utility semantics are owned by Agentic Mouse after the existing cell transport reaches the active mode.
...
- Emit one exact Shift-Command-4 key-down/key-up cycle through Core Graphics and
  let macOS own its configured destination and thumbnail behavior. The later
  build-154 learning supersedes this historical AppKit-monitor detail: track the
  bounded interaction with a listen-only Core Graphics event tap because the
  native overlay can consume successful selection events before AppKit sees them.
...
---
**Date:** 2026-09-04
**Trigger:** Ethan wanted top-level VS Code physical cells 5 then 4 to mirror the existing cells 8 then 7 Stage + Next gesture in the reverse direction.
**Fix:** Make physical cell 5 set an exact-source Previous hold marker, emit F17 only on an unconsumed release, and keep a one-second absolute expiry after release. A same-source physical-cell-4 press while held consumes the pending Previous and emits one non-repeating F19 Stage + Previous; one cell-4 press during the post-release window clears the expiry and emits the same F19 action.
**Guard:** Keep Corsair printed 4 and Razer printed 6 as the same canonical physical cell 4, and never combine holds across mice. Outside an active hold or its one-shot expiry, cell 4 must continue to arm the existing Copy / Paste wheel. A late F19 acts from the review position already reached by F17 and may move backward again; do not describe it as staging the change that was just left.
**Verification:** The focused exact-device generator tests and Default legend suite passed, and the complete clean gate passed 704 Swift tests, six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, generated-source freshness, both Karabiner lints, and packaging checks. Developer-ID-signed Agentic Mouse v1.0.165 (build 171) is installed with executable SHA-256 `aa5a4fbf874bc67da1275c0ad696dad86f44c941da37fae45474f8fdb1c6255f`; its iCUE framework retains SHA-256 `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe`. The live Karabiner configuration changed deliberately from `0eaa528e9b8e91d39281255618e953ad71c9e623bee0052cfe4a556428131721` to `558cbb4039b06d5ed7c5cfd6cfbbcff3b040d6c89c166ce1882844f2dc00e253`, preserving all three non-Agentic rules. Literal physical acceptance still requires one 5-then-4 press on each mouse; the first post-launch real-input gate also remained pending at installation handoff rather than being bypassed synthetically.
---
## 2026-09-04 — Separate Default YouTube click, speed hold, and wheel intent

**Request:** Hold the shared Default YouTube button (Corsair 6 / Razer 4) for temporary 2× speed, with no five-second seek on long-hold release.

**Fix:** `YouTubeScrubHoldController` classifies the existing exact-source press/release stream at 350 ms. It shares `ChromeYouTubeSpeedHoldController`'s opaque speed lease without participating in sticky double-click detection. An ordinary long-hold release restores the exact prior playback rate. Any wheel input, including filtered or ambiguous input, ends speed before scrub/volume dispatch; same-source cell 5 inhibits speed regardless of press order. Duplicate press cannot reset the threshold. Mode change, disconnect, lock, sleep, monitor stop and teardown cancel without a rewind. A short unused click retains −5 seconds.

**Guard:** Classify release from monotonic elapsed time, not whether a timer happened to run. A busy run loop can deliver release before its due callback; that must still suppress the click. Keep both mice and both mode entry points on the same lease so one hold cannot supersede another's saved playback rate. Notification feedback says requested; it is not confirmation of actual video speed.

**Verification:** Main-source clean gate passed 722 tests and all bridge/generator/packaging checks. The isolated installation source is commit `40b61fd` (installed v1.0.165/build171) plus only this feature's source/test changes; it excludes the pending Spotify, captured-origin staging, and unrelated dirty Tracks changes. That candidate passed 715 tests and all remaining checks. Two initial runs hit different existing main-queue timing assertions in screenshot/HUD tests; a third unchanged run passed. Eleven new deterministic hold tests pass, including delayed timer delivery, duplicate press, modifier/wheel priority, two mice, teardown, and non-sticky reuse.

**Installed verification:** On Ethan's explicit restart request, the exact enabled `Quit Agentic Mouse` item was resolved under the installed PID's `AXExtrasMenuBar` and its advertised `AXPress` action invoked. Both app and supervisor exited before replacement. Stable Developer-ID v1.0.166/build172 is installed with executable SHA-256 `a3d355c4fd187d11972e76a90faaaff21f68ddf1d0f240ff132b6d0ff5b55eea`; strict signature verification passed, the new runtime owns the command socket, and its supervisor is running. Live Karabiner, app config, SDK and VS Code keybindings remain byte-identical. The old bundle is retained under ignored `Rollbacks/AgenticMouse-v1.0.165-build171-before172.app`.

**Visual and physical boundary:** Immediately before Quit, Ethan had closed both legends. One temporary Corsair Default legend toggle after relaunch produced a visible build172 HUD on all three full-display pixel captures; the panels were subsequently absent during the user's live use. The new Hold 2× label is readable, but the trailing Wheel text is ellipsized at the current scale. Physical playback-rate, release restoration, no-seek and wheel acceptance remain unverified; no playback command was sent during installation.

**Install lesson:** Window-only UI control returned `noWindowsAvailable` or timed out for this menu-bar-only app. The native `AXMenuBar` attribute was absent, but `AXExtrasMenuBar` contained the exact enabled Quit item. When direct Accessibility control is authorized, target that observed menu item with bounded reads; require the exact installed PID/path and unique enabled advertised action, then prove both processes exited. Do not substitute generic AppleEvent Quit or speculative supervisor kills. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

## 2026-09-04 — Swap Codex chat-wheel and microphone cells

**Request:** Swap Corsair 6 and 11 only in Codex mode. Per the shared-cell contract, this also swaps Razer printed 4 and 11.

**Mapping:** Canonical cell 6 is Chats Selection + Wheel; cell 11 is Mute / unmute voice mic. `CodexMode.chatHistoryWheelCell` feeds the wheel resolver and diagnostic cell, and the Codex HUD derives its card from that resolver. The existing 80 ms ratchet filter, direction, command transports, automatic/manual entry, and every other mode remain unchanged. Both cells already have full exact-device press/release ingress, so no live Karabiner or iCUE change is needed.

**Verification:** The full main-source gate passed 723 tests, including an added coordinator test for both mice and both entry journeys: cell 6 arms and releases only the chat wheel, and cell 11 dispatches only the microphone press while preserving the mode. The isolated installation candidate passed 716 tests and all remaining checks; it retains the installed temporary YouTube 2× feature and excludes pending Spotify, captured-origin staging, and unrelated dirty Tracks changes.

**Installed verification:** Developer-ID-signed v1.0.167/build173 is installed with executable SHA-256 `6b060680947d0906c8bf9c96cc1157a74dcca719a1a78b637925b910fbd286d8`. Strict signature verification passed; the new runtime owns the command socket and its supervisor is running. The exact Quit menu item cleanly stopped both old processes before replacement; build172 remains under ignored `Rollbacks/AgenticMouse-v1.0.166-build172-before173.app`. Live Karabiner, app config, SDK and VS Code keybindings remain byte-identical.

**Visual and physical boundary:** Full-display pixel captures on all three connected displays showed both mirrored build173 Codex HUDs: Corsair 6 / Razer 4 is Chats Selection + Wheel, and printed 11 on both is Mute / unmute voice mic. The existing chat-wheel title remains ellipsized at the current scale. The initial socket probe was rejected while the macOS session-safety gate was inactive; after its real session-active notification, HUD-only manual entry succeeded through Utility cell 2 then Codex cell 1. Both task-opened modes were closed afterward and no Agentic Mouse HUD window remained onscreen. No chat navigation, microphone action or playback command was issued, so literal physical-button acceptance remains unverified. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

## Source-generated website simulator

- The public website is built with `make site` into `.build/site`. `ShowcaseExporter` replays the real `ModePickerCoordinator` with inert adapters; it must never load machine configuration, acquire a real hardware lease, or dispatch a Mac action. Native source changes reach Pages through the push-triggered workflow. Uncommitted changes and private settings are outside that publication boundary.
- A mode graph edge must preserve automatic/manual app targeting unless the native transition changes it. The shared Chrome websites submenu can be reached through either context, so do not freeze the first graph traversal's targeting state into that node.
- Project Three.js key-face corners after updating camera matrices to size HTML hit targets. Hide those targets when the faces point away from the viewer, suppress clicks after a drag, and restore their visibility in the no-WebGL grid. Verify mode, selected control, HUD, and action feedback together when scroll previews update selection.

## Reference-modelled website hardware

- Idle motion must keep the thumb faces in view: use a small side-view arc in the controls chapter, with free slow rotation confined to the hero. Share one visibility-aware 30 fps scheduler across the three views; input pauses for six seconds, keyboard focus and a held pointer extend that pause, and resuming uses elapsed visible time with a speed ramp. Do not rebuild native HUDs per animation frame. After a drag, ease the chapter back to its side view without firing the release click. Verify both hands' projected targets across a complete cycle, plus click/focus/pause/resume behavior in Chromium. (Codex task: 01a06ee5-4aa0-7a61-a029-704e5c44a8f2)
- Keep the two Blender-authored models separate: the Naga has a curved right-side thumb housing and continuous click/palm surface; the Scimitar SE has a left-side recessed yellow Key Slider and separate click plates. Ethan rejected generic rounded-shell stand-ins. Geometry stores printed numbers only; native exports own their physical-cell actions.
- A full mouse can hide a camera-facing key behind its shell. Raycast the complete model before exposing each HTML hit target, rather than relying on the face normal alone. Place Naga sockets against each key's local curved surface; a single planar grid visibly floats off the lower housing.
- Fit the Naga's complete keycaps and sockets below its lowest shoulder, including their tilt and bevels. Checking only key centres left the upper row crossing the shell seam. Verify the rebuilt GLB in the default side view and after pointer rotation; preserve native printed-number metadata and projected click targets.
- Use restrained, broad studio reflections for black ABS. RoomEnvironment's bright rectangular emitters made these models look metallic, and smooth cube normals made beveled flat keys look inflated. Weighted face normals and matching dark key materials preserve the actual manufacturing differences without silver key columns.
- Include both GLB and WebAssembly files in the site artifact and content digest. The hardware contract test checks twelve native printed buttons, a speech control, and the local decoder. Manually verify asset-load failure still exposes the product photographs and working HTML mode grid.
- On phones, place the reference photo, model, output panel and metadata in document flow. A shortened canvas with absolute overlays covered the mouse. Resize and project keys against the dedicated model box so moving the output below it preserves pointer alignment.
- Keep exact model picking accelerated with one shared BVH per geometry and `firstHitOnly` on each raycaster. A local benchmark of twelve rays against the unchanged 252k/272k-triangle assets fell from roughly 100 ms to below 0.3 ms per pass, with identical nearest hits across front/back poses. These are CPU picking timings, not whole-page frame rates. Coalesce chapter redraw requests into one animation frame.
- Do not draw an axis-aligned HTML focus rectangle over the angled 3D buttons. Highlight the actual key material for keyboard focus, retaining the ordinary outline only in the HTML fallback.
- A return to the controls chapter defaults to Corsair; an explicit Razer hero-key click keeps its own hand for that entry. Exclude the page's scroll-padding strip from visibility checks, because anchor navigation can leave the previous section technically intersecting it.

## Native HUD website presentation

- The source-generated website must show the actual mode HUD beside the clicked mouse, not only a textual action result. Both the chapter and full HUD section use `native-hud.mjs`; `ShowcaseExporter` resolves the native card colour roles and border treatments through the existing Swift presentation types. Keep native card titles, printed repair markers and Keypad cycles, with browser-only execution clearly identified.
- On phones, move the existing HUD below the model and reveal it with nearest scrolling after a physical-key action; keep the fallback model/grid and HUD in document flow too. On desktop, keep the action description below the model so it does not push the HUD or gestures out of the viewport. Preserve focused button nodes as modes change.
- Service the earliest pending Keypad deadline across both independent hand states. A click on the other hand must not cancel a pending letter's timer. Keep this event-driven; rebuilding the HUD during every Three.js frame would reintroduce scroll/drag lag.
...
## 2026-09-06 — Distinguish a signing-key failure from terminal access

A valid Developer ID certificate, a listed private key, and an unlocked/readable SecKeychain status do not prove signing can decode that key. During the Save release, the internal shell ran codesign normally, but securityd failed keychain decoding and SecKeyCreateSignature returned authentication failure. Earlier loginwindow logs showed a failed login-keychain/keybag unlock before this work. Inspect the exact codesign and loginwindow errors before attributing a failure to the model, shell access, certificate expiry, or missing app permission. A Computer Use refusal for the separate native Terminal app does not disable internal shell execution. Preserve the installed signed app and the tested candidate; do not reset TCC, change key ACLs, or recreate the keychain to work around a keybag failure. After Ethan performed one typed-password screen unlock, the same internal signing script succeeded immediately and security show-keychain-info worked again. No certificate, keychain ACL, TCC, or permission changes were needed. This verifies recovery for this incident without proving its original trigger or ruling out recurrence.

## 2026-09-06 — Scope Close to Keys mode

Keys cell 2 uses the shared native Keys output map for one non-repeating Command-W on both mice. The existing generator excludes native Keys cells from generic routing; keep cell 2's app-child Exit and top-level app-mode entry unchanged by scoping Close to the Keys page. Native selectNative feedback must leave Keys open and must never emit a second shortcut. Command-W follows the focused app's close-tab/document/window behavior.

## 2026-09-06 — Codex voice shortcut and explicit mode identity

The installed Codex 26.901.51231 global `realtimeVoice` handler calls `toggleGlobalHotkeySession`: without an active/reserved call it requests a new thread with source `global_hotkey_new_thread`; with a call reserved it stops the session or cancels startup. OpenAI's commands reference documents Control-Shift-V. The existing native cell-12 mapping already sends that shortcut through the accepted layout-aware exact-device route. Do not replace it with the separate composer voice command or clear the physical repair marker from code inspection alone.

Codex mode now uses the explicit blue RGB(32, 128, 255) and a 4.5-point panel outline. Route both automatic frontmost-app and manual selector identity resolution through `AppSpecificTarget.definition(iconAccent:)`; changing the static colour alone leaves the installed icon sampler able to override it. Other apps retain their icon-derived identity. The two Codex Exit cells use the shared installed-icon backdrop, and the website exports that same artwork role and outline width.

Verification: the installed-source candidate passed all 729 Swift tests; the clean publication source passed 724 Swift tests and its complete build/bridge/generator/install/packaging/site gate, including 15 browser-simulation tests. The first installed-source suite hit the existing Space-reattachment timing test; the unchanged serial rerun passed. Stable Developer-ID v1.0.177/build183 was installed and restarted. Full-display captures showed the thicker blue perimeter and icon-backed Exit cards in both mirrored HUDs on all three connected displays. Config, Karabiner, Codex shortcuts and embedded SDK remained byte-identical. No voice call was started or ended during verification; physical Voice button acceptance remains separate.

## 2026-09-06 — More forgiving screenshot double press

Ethan requested a slightly slower screenshot double-click gesture while learning the left-handed Razer. Increase only `SelectedAreaScreenshotController.doublePressInterval` from 280 to 400 milliseconds for both mirrored screenshot buttons (Corsair 3 / Razer 1). The single screenshot starts 120 milliseconds later; the second press still pastes immediately when a saved screenshot is available. Other double-click families and macOS's system setting remain independent. Existing same-source, cancellation, pending-save, clipboard-ownership and session-gate tests cover the unchanged classifier behavior.

## 2026-09-06 — Separate screenshot clicks from directional Codex pinning

Codex 26.901.51231 exposes only a combined Command-Option-P pin toggle as a keyboard command, while its task menu offers explicit Pin/Unpin actions. The live main header exposes one Chat actions popup and a title button under its fixed h-toolbar; embedded browser content is a separate AXWebArea and must be excluded. AXExpanded remains readable even when it is absent from AXCopyAttributeNames. A bounded read-only production-header probe resolved the live header without changing focus or opening a menu. Require that exact window/header throughout a gesture, and treat successful AXPress only as a request, not confirmed persisted pin state.

Cell 3 reuses the shared wheel release record: a wheel-free release calls the existing screenshot controller, while any wheel input (including ambiguity and phased pass-through) suppresses screenshot. A once-per-hold policy prevents bursts from repeatedly opening menus; duplicate press commands must not re-arm the hold. Test pin-on-pinned and unpin-on-unpinned as no-ops, ambiguous menus, focus/lock cancellation, and both mice's physical wheel polarity. UI consumption and physical button acceptance remain separate from these automated checks.

The pin menu request additionally checks its originating mouse coordinator while awaiting the menu, so leaving that source’s Codex mode cancels the action even if the other mouse still has Codex mode open. Async action tests wait for callback completion rather than assuming a fixed 100 ms delay is sufficient on a loaded CI runner.

## 2026-09-07 — Resolve the Codex title after its optional project selector

A project chat inserts a Project popup before the task-title group beside Chat actions. Choosing the first header child rejected these valid windows while the earlier projectless probe passed. Select the header title group by its AXGroup role, keeping the unique named button, exact main-toolbar ancestry and window/foreground guards. A read-only production initializer reproduced the project failure (zero title buttons) and recognized that same window after the repair. The held Screenshot card must show Pin/Unpin · Wheel and restore Screenshot immediately on release. Keep the physical failure marker until Ethan confirms pinning; a successful header probe does not prove menu consumption.


- 2026-09-07: macOS CI's `/bin/bash` is Bash 3.2. With `set -u`, expanding an empty optional argument array fails before the build. Use the guarded array expansion in the packager and keep first-install preflight tests in the release gate. The universal signed build was verified locally; public notarization remains pending repository secrets.
- 2026-09-07: a source-only Spotify controller does not make a physical button work. Extra Utilities needs the shared physical-cell action, coordinator outcome and AppDelegate asynchronous route in the installed candidate. Verify the bottom player's radio context before reporting that playback changed; a visible radio page is insufficient.
- 2026-09-07: screenshot paste is an explicit once-per-hold wheel action. Any observed wheel input, including an inactive direction or ambiguous event, must suppress capture on release. Starting a fresh capture cancels a pending deferred paste from an older capture.

- 2026-09-07: the settled HUD Space callback removes panels and enqueues recreation on the next main-queue turn. Tests must drain that queue before inspecting visibility; sleeping until the callback is due can observe the temporary empty interval under load. Retain original windows while comparing ObjectIdentifiers so allocator reuse cannot masquerade as the same panel. The corrected seven lifecycle tests and full native suite pass.

- 2026-09-07: keep the photo-based room beta at `beta.html` alongside the main walkthrough. Reuse the committed Swift export, simulator, native HUD and hardware models; its separate code-review example describes the personal 300 ms hold-on-release workflow. The original room portrait remains a proportioned plane so camera motion does not warp the face or invent unseen geometry. Verify Escape during a held review gesture and during dictation, plus already-paused video ownership. Serve a completed site build before browser navigation: rebuilding `.build/site` replaces that directory and can interrupt an in-flight local module load.

- 2026-09-07: room-camera hotspot buttons use flex layout, which overrides the browser's default display for the hidden attribute. Give hidden hotspots explicit visibility and disable pointer events while retaining measurable widths; use each full label's width when hiding it near an edge. Check a touch pan in both directions so off-screen labels disappear and return correctly.

- 2026-09-07: room dragging must track the pointer directly. Easing the pan, capping the room at 30 fps, and measuring every label between per-frame style writes made the photo camera feel delayed. Cache hotspot widths on resize and render the proportioned photo at CSS-pixel resolution. Feature selection centres the camera on that item and expands its controls; closing restores the exact prior view. Manually verify entry, drag, focused controls and return in Chrome after the final edit, including the narrow layout.

- 2026-09-08: The galaxy background must not use the mouse-model idle scheduler: its pointer-move pause froze the animation for six seconds during interaction. Use a separate visibility-aware loop and projected-screen pointer forces so individual stars respond under the cursor. Keep stroke history bounded, clear it when scroll/resize changes screen coordinates, and verify both the changing shader output and mouse-model controls in the built-in browser. Reference posters and interaction labels cannot verify another site's actual pointer physics when its animation chunks fail to load. (Codex task: 01a07944-b48e-7e43-8c2f-34b9cfe3df70)

- 2026-09-09: The Agentic Mouse website uses ordinary document scrolling. Ethan rejected the pinned controls tour and scroll-driven animation; do not restore oversized scroll scenes, automatic selection while scrolling, or a chapter progress bar. Keep model rotation and button interactions independent of page scrolling, and verify desktop and mobile scrolling through the complete controls section.

- 2026-09-09: After splitting Ethan’s setup into its own website, keep Agentic Mouse’s browser title, social title and web manifest names app-specific; the shared mouse-grid favicon was already correct. The YouTube integration needs both the VoiceInk YouTube Bridge Chrome extension and its native macOS helper, separate from the public VoiceInk++ source. Document the private-source availability boundary and conditional setup steps without linking visitors to a private repository.
