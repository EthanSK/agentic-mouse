...
**Install boundary:** Stable Developer-ID candidate v1.0.166/build172 is packaged and retained under ignored `Rollbacks/Candidates/`. Computer Use failed with `noWindowsAvailable` and then repeated `timeoutReached` responses, including after resetting the connection, before the dedicated Quit menu could be invoked. No replacement or forced termination occurred. Ethan has been asked to choose `Quit Agentic Mouse`; confirm its main process and supervisor are stopped before installing. Keep live Karabiner, app config, SDK and VS Code keybindings unchanged. Preserve prior HUD state, verify all-display pixels after relaunch, and obtain physical speed/restore acceptance before declaring live completion.
