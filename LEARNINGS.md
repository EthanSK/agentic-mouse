...
**Trigger:** Ethan reported that VoiceInk could replace the pasteboard between a selected-area capture and the Screenshot button's rapid double-press paste. He also reported that externally dismissing the native crosshair could leave the HUD stuck in its capture state.

**Finding:** The native Shift-Command-4 interaction saves the authoritative image to the configured Screenshot directory, so keeping a screenshot resident on the general pasteboard after capture unnecessarily competes with VoiceInk. Arbitrary macOS apps still accept an image at the current cursor through Command-V rather than through a universal direct file-insertion API, so the correct boundary is a short pasteboard lease at the moment of paste, not no pasteboard use at all. A plain click without a drag dismisses the native crosshair without writing a file. The public `CGWindowList` owner `screencapture` exposes the bounded interaction while it is active, including dismissals that do not reach the global event monitor. Live v1.0.145 verification also proved that closing the native overlay can strand fully rendered HUD panels on stale display Spaces even when their model remains active.

**Fix:** Snapshot the configured Screenshot directory before capture, resolve and retain the exact new saved image path without changing the pasteboard, and on rapid double-press snapshot every current pasteboard item before installing the exact file URL, original image representation, TIFF, and a private ownership marker. Send one process-targeted hardware-shaped Command-V to the then-frontmost app, then restore the old pasteboard after 250 ms only while the marker and change count still own it; a newer VoiceInk, user, or application write wins. Repeated screenshot pastes share the pre-first-paste snapshot. Classify plain click as cancellation, drag-and-release as completion, and Escape as cancellation while retaining the original 60-second saved-file resolver for a real drag. When the native overlay ends, recreate only explicitly active HUD presenters immediately and once after 500 ms so their panels reattach to every current display Space. (Codex task: 01a039f7-873c-7c30-b3dc-af8a6724ace5)

**Guard:** Do not take a second screenshot, hold the screenshot on the general pasteboard between capture and paste, restore over a newer clipboard write, infer completion from mouse-up without distinguishing a drag, shorten the real capture's slow-destination allowance, or treat AX window counts/direct captures as current-Space visibility proof. Hidden legends must remain hidden when the screenshot overlay closes.

**Verification:** The complete clean gate passed 676 Swift tests, including 22 Screenshot controller tests, six AppKit Space-lifecycle tests, and six clipboard-preservation tests, plus six Musixmatch tests, six VS Code bridge tests, 17 Karabiner generator tests, both Karabiner lints, packaging/version contracts, and shell syntax. The updated `configure-mice` skill passed validation. Developer-ID-signed Agentic Mouse v1.0.146 (build 152) is installed as main PID 45285 with supervisor PID 45293, Accessibility granted, iCUE connected, self-recovery enabled, and exclusive mode-0600 ownership of the command socket. Its executable SHA-256 is `f6e44212969f22c2e251f252946c53280a58a9651c70248063a5aeedaeae26a0`, CDHash is `4fbeb7e3f95e0fe57a81f4932021b1af61fcc6c6`, and embedded iCUE SDK SHA-256 remains `48bbc94bed670d036af8e1acca0017449b36d7c6d1e15dafe0791b42b6be84fe`. A live native plain-click cancel reduced `screencapture` from five surfaces to zero and left all three HUD panels onscreen; full-display pixel captures `/tmp/agentic-mouse-v146-after-cancel-d1.png` through `d3.png` visibly show the idle `Screenshot` card and v1.0.146 legend on every current display Space. The temporary Razer legend was then closed, leaving zero AX windows. OBS++'s active mux remained PID 56552 throughout. Exact prior builds are preserved at `Rollbacks/AgenticMouse-v1.0.144-build150-before-v1.0.145.app` and `Rollbacks/AgenticMouse-v1.0.145-build151-before-v1.0.146.app`, with the replaced applications also recoverable from Trash. Literal physical screenshot creation and paste into Ethan's chosen target remain pending acceptance because this verification deliberately did not paste into a user app.
